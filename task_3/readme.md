...
Since the assignment required identification of Shiga toxin genes and their genetic context, the annotation file `scaffolds.gff3`, generated by Bakta was parsed using a custom [Python script](). The script extracts all CDS features, retrieves the locus tag, product description, start and end coordinates, and saves the results as a TSV table.

After running the script,the resulting `gene_table.tsv` was filtered for entries related to Shiga toxin or stx genes:
```bash
